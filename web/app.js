...
if (configPanel && configPanel.dataset.hydrated !== '1') {
  ...
  if (configActions) {
    const actionButtons = Array.from(configActions.querySelectorAll('button'));

    initializeRuntimeButton =
      actionButtons.find((button) => button.dataset.action === 'initialize') ||
      actionButtons.find((button) => /initialize/i.test(button.textContent)) ||
      actionButtons.find((button) => button.classList.contains('secondary-action'));

    generateButton =
      actionButtons.find((button) => button.dataset.action === 'generate') ||
      actionButtons.find((button) => /generate|run\s*test/i.test(button.textContent)) ||
      actionButtons.find((button) => button.classList.contains('primary-action'));

    if (initializeRuntimeButton) {
      initializeRuntimeButton.dataset.action = 'initialize';
      initializeRuntimeButton.dataset.config = 'actions';
      initializeRuntimeButton.type = 'button';
      if (!runtimeReady) {
        const label = initializeRuntimeButton.textContent.trim();
        if (!/initialize/i.test(label)) {
          initializeRuntimeButton.textContent = 'Initialize Runtime';
        }
      }
    }
    ...
    if (generateButton) {
      ...
      generateButton.dataset.config = 'actions';
