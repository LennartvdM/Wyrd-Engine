...
async function loadSources(instance) {
  ...
  const manifest = JSON.parse(await fetchText(new URL('manifest.json', base)));
  const files = (manifest.files || []).filter((f) => f.path.startsWith('flock/'));
  if (files.length === 0) {
    throw new Error('py/manifest.json lists no flock/ sources — run scripts/build_pyodide_manifest.mjs');
  }
  progress('sources', `${files.length} files`);

  const sources = await Promise.all(
    files.map(async (f) => ({ path: f.path, text: await fetchText(new URL(f.path, base)) }))
  );

  instance.FS.mkdirTree('/flock_app/flock');
  for (const { path, text } of sources) {
    instance.FS.writeFile(`/flock_app/${path}`, text);
  }
